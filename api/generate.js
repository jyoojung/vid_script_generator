...
3. card_title: 해당 숏폼 기획안이 다루는 '핵심 콘셉트나 기획 의도'를 10자 이내로 짧게 작성할 것. (예: 야행성 맞춤 수분 공급)
4. on_screen_titles: 철저하게 유튜브/릴스 조회수를 유도할 수 있는 '어그로성 후킹 자막(초기 3초용)' 5가지로만 구성할 것.
   ※ [핵심 금지사항]: \`card_title\`과 \`on_screen_titles\`의 각 항목(특히 1번 제목)은 절대로 내용이 겹치거나 동일한 구문/단어로 반복 작성되어서는 안 됨! 두 항목은 '단정한 기획 의도'와 '자극적인 클릭 유도'라는 서로 완전히 다른 목적을 가지므로 철저히 분리해서 다르게 작성할 것.
5. highlight_keywords: 자막 편집 시 강조하거나 해시태그(#)로 활용할 단어 3~5개를 추출할 것. 무조건 2~6자 이내의 짧은 '명사형' 단어로 압축하며, 시청자의 이목을 끄는 직관적인 단어(예: 칼퇴근, 노코드, 비용절감, 충격 등)를 우선 선정할 것.
6. script_snippet (숏폼 내용 요약): 단순히 원본 대사를 그대로 잘라오지 말 것. 해당 구간에서 화자가 말하고자 하는 핵심 기획 의도와 스토리를 편집자가 완벽히 이해할 수 있도록, 원본 맥락을 바탕으로 2~3문장의 '완성된 줄글(요약본)'로 다듬어서 작성할 것. 말하다 끊긴 느낌이 없도록 반드시 온점(.)으로 끝나는 완결된 문장으로만 구성하며, 읽기 편하게 문장 단위로 줄바꿈을 적용할 것.
7. edit_guide: 편집자가 실무에서 직접 체크하며 작업할 수 있도록 [효과음/BGM, 컷편집/호흡, 화면/자막 모션] 3가지 영역으로 나누어, 당장 적용할 수 있는 구체적인 팁을 국문으로 1개씩 작성할 것. (영문 표기 제외)
8. Format: 모든 결과물은 한국어이며, 반드시 아래의 JSON 배열 형식으로만 대답할 것.
